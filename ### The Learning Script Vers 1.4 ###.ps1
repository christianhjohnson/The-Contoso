...
# Asks it you want an .ISO installed, if so prompts for location.  If location is not known it searches and gives a list of all .ISOs and prompts to enter directory and file name.
$ISOAssociate = Read-Host -Prompt "Do you want to associate a .ISO to the VM? (Y or N)"
if ($ISOAssociate -contains "Y") {
    $ISOLocate = Read-Host -Prompt "Do you know the location of the .ISO file?"
    }
    if ($ISOLocate -contains "Y") {
        $ISODirectory = Read-Host -Prompt "Enter the directory to the .ISO file (Ex: C:\ISOs)"
        $ISOFile = Read-Host "Enter the ISO to install"
        }
    if ($ISOLocate -contains "N") {
        ...
        $ISODirectory = Read-Host "Enter the Directory Name for the ISO (Ex: C:\ISOs)"
        $ISOFile = Read-Host "Enter the ISO to install (Ex. Server2016.iso)"
        }
